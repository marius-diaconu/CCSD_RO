...
{
    public class EditTaskViewModel : Conductor<object>
    {
        private Tasks task;
        ...
        private short _taskInterval;
        private DateTime _taskStartDate;
        private DateTime _taskEndDate;
        private string _taskTime;
        private string _taskLastRun;
        private string _taskNextRun;
        private string _stopTaskBtnText;

        public EditTaskViewModel(string task_uuid)
        {
            task = new Tasks();
            task = task.FindByUuid(task_uuid);
            this.SetTask();
        }

        public string TaskUuid
        {
            get { return _taskUuid; }
            set
            {
                _taskUuid = value;
                NotifyOfPropertyChange(() => TaskUuid);
            }
        }
        ...
        public string TaskLastRun
        {
            get { return _taskLastRun; }
            set
            {
                _taskLastRun = value;
                NotifyOfPropertyChange(() => TaskLastRun);
            }
        }

        public string TaskNextRun
        {
            get { return _taskNextRun; }
            set
            {
                _taskNextRun = value;
                NotifyOfPropertyChange(() => TaskNextRun);
            }
        }

        public string StopTaskBtnText
        {
            get { return _stopTaskBtnText; }
            set
            {
                _stopTaskBtnText = value;
                NotifyOfPropertyChange(() => StopTaskBtnText);
            }
        }
        ...
        private void SetTask()
        {
            if (task.Exists())
            {
                ...
                DirectoryClearCheckbox = task.task_action == 0 ? true : false;
                DirectoryCopyCheckbox = task.task_action == 1 ? true : false;
                OneWaySyncCheckbox = task.task_action == 2 ? true : false;
                BothWaysSyncCheckbox = task.task_action == 3 ? true : false;
                WithSubDir = Convert.ToBoolean(task.sub_dirs);
                SourceDirectoryPath = task.source_path;
                DestinationDirectoryPath = task.dest_path;
                OneTimeTask = (task.task_type == Tasks.Type.ONE_TIME.ToString()) ? true : false;
                DailyTask = (task.task_type == Tasks.Type.DAILY.ToString()) ? true : false;
                WeeklyTask = (task.task_type == Tasks.Type.WEEKLY.ToString()) ? true : false;
                MonthlyTask = (task.task_type == Tasks.Type.MONTHLY.ToString()) ? true : false;
                TaskInterval = task.task_interval;
                TaskStartDate = DateTime.Parse(task.task_start_date);
                TaskEndDate = DateTime.Parse(task.task_end_date);
                TaskTime = task.task_time;
                TaskLastRun = string.IsNullOrEmpty(task.task_last_run) ? "Nu a rulat încă" : task.task_last_run;
                TaskNextRun = string.IsNullOrEmpty(task.task_next_run) ? "Nu există" : task.task_next_run;
                StopTaskBtnText = Convert.ToBoolean(task.task_stopped) ? "Activează Task" : "Dezactivează Task";
            }
        }
        ...
        public void SelectSourceDirectory()
        {
            ...
            if (!string.IsNullOrEmpty(source_dir.SelectedPath))
            {
        ...
        public void SelectDestinationDirectory()
        {
            ...
            dest_dir.ShowDialog();
            if (!string.IsNullOrEmpty(dest_dir.SelectedPath))
            {
        ...
        public void UpdateTaskButton()
        {
            if (string.IsNullOrEmpty(TaskName))
            {
                MessageBox.Show("Vă rugăm introduceți un nume descriptiv pentru acest task, " +
                    "înainte de a apăsa butonul [ Actualizează Task ]", "Avertisment",
            ...
            {
                MessageBox.Show("Este necesară selectarea dosarului sursă, înainte de a apăsa butonul [ Actualizează Task ]",
            ...
            if (DirectoryCopyCheckbox || OneWaySyncCheckbox || BothWaysSyncCheckbox)
            {
                if (string.IsNullOrEmpty(DestinationDirectoryPath))
                {
                    MessageBox.Show("Este necesară selectarea dosarului de destinație, înainte de a apăsa butonul " +
                        "[ Actualizează Task ]", "Avertisment", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            }
            ...
            if (!OneTimeTask && !DailyTask && !WeeklyTask && !MonthlyTask)
            {
                MessageBox.Show("Este necesară selectarea uneia dintre opțiunile listate în Repetare: o dată, zilnic, " +
                    "săptămânal, lunar, înainte de a apăsa butonul [ Actualizează Task ]", "Avertisment",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            else
            ...
            {
                MessageBox.Show("Este necesară setarea orei și a minutului, înainte de a apăsa butonul " +
                    "[ Actualizează Task ]", "Avertisment",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            else
            ...
            if (task.Save())
            {
                ...
                MessageBox.Show("Task-ul a fost actualizat cu succes", "Succes",
        ...
        public void StopTaskButton()
        {
            if (task.IsStopped())
            {
                ...
                if (task.Save())
                {
                    ...
                    MessageBox.Show("Task activat cu succes!", "Succes",
                            MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
            ...
            else
            {
                ...
                if (task.Save())
                {
                    ...
                    MessageBox.Show("Task dezactivat cu succes!", "Succes",
                            MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
